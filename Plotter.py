...
def plot_SIR(pandemic_info, as_percentage=True):
    ...
    S = pandemic_info["S"]
    I = pandemic_info["I_inc"] + pandemic_info["I_symp"] + \
    ...
    ax.stackplot(
        np.arange(R),
        I, S, death, surv,
        colors=["darksalmon", "darkslategray", "red", "lightskyblue"],
        labels=["I", "S", "Dead", "Surv"]
    )
    ax.legend()
...
def plot_intervals(y):
    ...
    for i in reversed(range(n_intervals)):
        p = (i + 1) / n_intervals
        plt.fill_between(
            x, *np.quantile(y, q=[0.5 - p / 2, 0.5 + p / 2], axis=0),
            color=cm.jet(norm(i))
        )
