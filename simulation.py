...
if SIR:
    sampler = Sampler(  # SIR ###########
        avg_people_met=5, contagion_prob=0.04, crit_prob=0.0  # DONT CHANGE (0)
        , death_prob=1.0  # UNUSED
        , symp_prob=1.0  # DONT CHANGE (1)
        , fraction_symp_out=0.5        # , avg_time_inc=0.0 # DONT CHANGE (0)
    ...
    sampler = Sampler(  # EXTENDED SIR ##########
        ...
        # , avg_time_symp=7.5
        # , avg_time_no_symp=7.5
        avg_people_met=20, contagion_prob=0.04, crit_prob=0.03, death_prob=0.22, symp_prob=0.9, fraction_symp_out=0.3, avg_time_crit=10
    )
...
if plot_data:
    ...
    if SIR:
        ...
        Plotter.plot_hospitalized_people(
            result['I_symp']*sick_people_on_hospital_fraction, hospital_beds)
    else:
